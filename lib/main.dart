...
class HomeState extends State<Home> with TickerProviderStateMixin {
  ...
  Widget emptyList() {
    return Center(child: FadeTransition(opacity: emptyListController, child: Text('No items')));
  }
  ...
  Widget buildListTile(item, index) {
    return ListTile(
      ...
      title: Text(
        item.title,
        key: Key('item-$index'),
        style: TextStyle(color: item.completed ? Colors.grey : Colors.black, decoration: item.completed ? TextDecoration.lineThrough : null),
      ),
      trailing: Icon(
  ...
  void addItem(Todo item) {
    ...
    items.insert(0, item);
    if (animatedListKey.currentState != null) animatedListKey.currentState.insertItem(0);
  }
